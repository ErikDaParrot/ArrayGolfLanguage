...
windows = lambda x, y: view_as_windows(np.array(x), tuple(y if type(y) == list else [y])).tolist()
...
where = lambda x: [i if np.array(x).ndim == 1 else encode_mixed_radix(x.shape, i) for i, o in enumerate(ravel(x)) if o]
...
def encode(x, y):
  if type(y) == list:
    r = [encode_mixed_radix(x, i) for i in y]
